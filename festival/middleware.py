...
class FestivalLocaleMiddleware:
    ...
    en_urls = [re.compile(p) for p in ['^/author/', '^/visitor/', '^/journalist/', '^/author', '^/visitor', '^/journalist']]
    cs_urls = [re.compile(p) for p in ['^/tvurce/', '^/navstevnik/', '^/novinar/', '^/tvurce', '^/navstevnik', '^/novinar']]
